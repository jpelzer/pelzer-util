...
public abstract class OverridableFields implements Serializable {
  ...
  protected void init(String domain) {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      try {
        if (isArray) {
          // We need to grab all the values for the array...
   ...
   * @throws Exception
   *           if there is a problem converting the String to the given type.
   */
  private Object getObjectForString(Class<?> type, String value) throws Exception {
    if (type.isArray())
      type = type.getComponentType();
    // debug.debug("getObjectForString: type=" + type.toString() + " " +
    ...
    if (type.equals(Boolean.TYPE)) {
      String v = value.toString().toUpperCase().trim();
      return new Boolean(v.startsWith("T") || v.startsWith("Y") || v.startsWith("1") || v.startsWith("ON"));
    }
    if (type.equals(ObfuscatedString.class))
      return new ObfuscatedString(value.toString());
    // Default to String
    if (type.equals(String.class))
      return value;
    throw new Exception("Don't know how to handle a class type of '" + type.toString() + "'");
  }
